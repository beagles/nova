...
environment_template = [
    ...
    'VIF_OVS_INTERFACE=%(ovs_interfaceid)s',
...
def generate_environment(vif):
    ...
    if details:
        for k, v in details.iteritems():
            result.append('VIF_DETAIL_%s=%s' % (k, v))

    return result

def verify_environment(expected_env, generated_env):
    ...
    for k, v in expected.iteritems():
        self.asserEqual(v, generated.get(k, 'XXX_INVALID_VALUE'))

    for k in generated:
        self.assertTrue(expected.has_key(k))
